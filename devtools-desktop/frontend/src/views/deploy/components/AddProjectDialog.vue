    ...
    <div class="add-project">
      ...
      <template v-if="mode === 'scan'">
        <div class="add-project__toolbar">
          ...
          />
          <BaseButton variant="ghost" size="sm" @click="emit('toggle-all-scan', true)">全选</BaseButton>
          <BaseButton variant="ghost" size="sm" @click="emit('toggle-all-scan', false)">全不选</BaseButton>
      ...
      <template v-else>
        ...
        <p class="add-project__count add-project__count--block">已选 {{ browseChecked.size }} 个</p>
      </template>
...
.add-project__count--block { margin: 0; }
