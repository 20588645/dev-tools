...
function preflightCheck(serverConfig, remotePath) {
  return new Promise((resolve) => {
    ...
    // 外层总超时必须对齐可配连接超时，并加 buffer 覆盖 SFTP 初始化 + 远程 stat；
    // 否则用户把连接超时调到 >60s 时，预检仍会被写死的 60s 砍断，与内层 readyTimeout 自相矛盾
    const connTimeoutMs = db.getConnTimeoutMs();
    const overallTimeoutMs = connTimeoutMs + 15000;
    const timer = setTimeout(() => {
      fail({ success: false, error: `连接超时 (${Math.round(overallTimeoutMs / 1000)}s)` });
    }, overallTimeoutMs);
...
router.post('/build', async (req, res) => {
  ...
  // 顶层 try/catch：res.json 已先行返回，此后的异步流程（build 等）一旦抛错，
  // 若不兜底，registerJob 登记的活跃任务永不 unregister，前端卡片会永久卡在 busy。
  // 出错时必须清理 activeJobs 并经既有 broadcast 通道补发 done(fail) 终态。
  try {
    // 任务概览
    onLog('info', '╔══════════════════════════════════════════╗');
    onLog('info', '║           🚀 构建任务启动                ║');
    ...
    console.error('[Deploy] /build 异步流程异常:', err.message);
    try { onLog('error', `❌ 构建任务异常中止: ${err.message}`); } catch {}
    const record = {
      id: deployId, timestamp: new Date().toISOString(), projectName,
      modules: buildModulesLabel,
      serverId: null, serverName: '—', remotePath: '—',
      status: 'fail', type: 'build-only', duration: '0s', logs
    };
    appendHistory(record);
    unregisterJob(deployId);
    broadcast('status', { id: deployId, phase: 'done', ...record });
...
router.post('/start', async (req, res) => {
  ...
  // serverId -> conn：预检阶段建立的连接，部署各阶段复用；提到 try 外层声明，
  // 便于异步流程异常时在 catch 中统一关闭，避免连接泄漏
  const serverConns = new Map();

  // 顶层 try/catch：res.json 已先行返回，此后的异步流程（预检/构建/上传）一旦抛错，
  // 若不兜底，registerJob 登记的活跃任务永不 unregister，前端卡片会永久卡在 busy。
  // 出错时必须关闭残留连接、清理 activeJobs 并经既有 broadcast 通道补发 done(fail) 终态。
  try {
    // 任务概览
    onLog('info', '╔══════════════════════════════════════════╗');
    ...
    console.error('[Deploy] /start 异步流程异常:', err.message);
    try { onLog('error', `❌ 部署任务异常中止: ${err.message}`); } catch {}
    for (const conn of serverConns.values()) { try { conn.end(); } catch {} }
    const record = {
      id: deployId, timestamp: new Date().toISOString(), projectName,
      modules: deployModulesLabel,
      serverId: serverIds[0], serverIds: serverIds, serverName: targetServers.map(s => s.name).join(', '),
      remotePath: remotePath || '', status: 'fail', type: 'deploy', duration: '0s', logs
    };
    appendHistory(record);
    unregisterJob(deployId);
    broadcast('status', { id: deployId, phase: 'done', ...record });
...
// type/status 为可选过滤条件，做白名单校验：history 表写入的取值仅有以下枚举，
// 传非法值直接 400，避免脏查询参数静默命中空集（虽已参数化、无注入风险）
const LAST_ALLOWED_TYPES = ['build-only', 'deploy'];
const LAST_ALLOWED_STATUS = ['success', 'fail'];
router.get('/last/:projectName', (req, res) => {
  const { projectName } = req.params;
  const { type, status } = req.query; // 可选过滤条件

  if (type !== undefined && !LAST_ALLOWED_TYPES.includes(type)) {
    return res.status(400).json({ error: `type 非法，仅允许: ${LAST_ALLOWED_TYPES.join(', ')}` });
  }
  if (status !== undefined && !LAST_ALLOWED_STATUS.includes(status)) {
    return res.status(400).json({ error: `status 非法，仅允许: ${LAST_ALLOWED_STATUS.join(', ')}` });
  }
