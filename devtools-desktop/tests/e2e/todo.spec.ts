...
test('mounts one Vue todo page in both themes without the retired runtime', async ({ page }) => {
  ...
  // redesign-v2：平铺任务行 + 状态分段筛选（默认「进行中」隐藏已完成）
  await expect(page.locator('.todo-task-row')).toHaveCount(2)
  ...
  await expect(page.getByText('升级依赖包', { exact: true }).first()).toBeVisible()
  await page.getByRole('tab', { name: '全部', exact: true }).click()
  await expect(page.locator('.todo-task-row')).toHaveCount(3)
  await page.getByRole('tab', { name: '进行中 2', exact: true }).click()
  await expect(page.locator('.todo-task-row')).toHaveCount(2)
...
test('searches, filters, creates and auto-saves the legacy-compatible content', async ({ page }) => {
  ...
  await page.getByRole('button', { name: '新建待办', exact: false }).click()
...
test('confirms incomplete child tasks before completing a parent and clears done tasks', async ({ page }) => {
  ...
  await expect(page.getByText('还有 1 个子任务未完成', { exact: true })).toBeVisible()
  await page.getByRole('button', { name: '一起完成', exact: true }).click()
