...
test('repairs an app path from the dedicated tool page', async ({ page }) => {
  ...
  const layout = await page.locator('#page-appfix').evaluate((root) => {
  ...
  const controlHeights = await page.locator('#page-appfix .app-fix-toolbar').evaluate((toolbar) => {
  ...
  await page.getByRole('button', { name: '开始修复', exact: true }).click()
  await expect(page.locator('#page-appfix .app-fix-result')).toContainText('已清除隔离属性，可以再试着打开这个应用。')
  await expect(page.locator('#page-appfix').getByText('/tmp/Demo.app')).toBeVisible()

  const overflow = await page.locator('#page-appfix').evaluate((root) => ({
...
test('copies an app off a mounted install image then ejects it', async ({ page }) => {
  ...
  await page.getByRole('button', { name: '开始修复', exact: true }).click()
  await expect(page.locator('#page-appfix .app-fix-result')).toContainText('应用还在安装镜像里')
  await page.getByRole('button', { name: '拷到应用程序并推出', exact: true }).click()
  await expect(page.locator('#page-appfix .app-fix-result')).toContainText('/Applications/Demo.app')
