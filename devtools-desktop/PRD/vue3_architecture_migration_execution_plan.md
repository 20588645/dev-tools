...
| 16 | Phase 9 | 旧架构清理与发布 | **进行中（P9-1～P9-5 已完成）** | P9-6 CM5/xterm → npm | 见 `pages/phase9/`；分批删 legacy / 旧 `src` / 污染 CSS；G7/G8 |
...
**部署历史子页已完成 PG0～PG5 自动验证**（七列 `BaseDataTable` + 筛选 + 批量选择 + 自动整理弹窗；修掉 H1 模块标签被压成单字符、H2 操作列被挤出可视区、H3 字符按钮无无障碍名、H4 复选框对比度过低，以及 H5 `getHistoryLog` 读错字段致日志恒空、H6 测试模式读写并删除正式库日志、H7 ISO 时间戳被当数字解析致时间恒显示占位——H7 同时影响项目总览的「最近部署」摘要。legacy History 整段已退役，`switchSubTab` 简化为只派发激活事件）。**待该子页 Tauri 验收，通过后删 `deploy.css` 并收口四项跨页耦合。**
...
**分批进度（见 `pages/phase9/`）**：P9-1 孤儿清理 ✅；P9-2 Toast/介绍 ✅；P9-3 桌面通知 + run WS 通知 ✅；P9-4 删 `app.js` ✅；P9-5 WS/API 归 Vue、删 `websocket.js`/`api.js` ✅；P9-6～P9-8 待办。

#### 删除清单

- [x] 删除无消费者 vendor：`sortable.min.js`（P9-1）
- [x] 删除 `window.sendDesktopNotification` / `__runActiveJob` 与 app.js run 通知路径（P9-3）
- [x] 删除旧 `src/js/app.js`（P9-4）
- [x] 删除 `src/js/api.js` / `src/js/websocket.js`；`window.WS` → `services/realtime.ts`（P9-5）
...
| 13 | 文件编辑 | `views/editor/`、`stores/editor.ts`、离开守卫桥；旧 js/css 已删；`assessment.md` + `decision.md` | 高 | CodeMirror、未保存状态 | Phase 7-1 | **PG5 已通过** | 保持回归；下一页为终端 |
| 14 | 快捷命令 | `views/terminal/`、`terminal-runtime-service`；旧 js/css 已删；`assessment.md` + `decision.md` | 很高 | Xterm、PTY、WebSocket、WebGL | Phase 7-2 | **PG5 已通过** | 保持回归；下一阶段 Phase 8 |
