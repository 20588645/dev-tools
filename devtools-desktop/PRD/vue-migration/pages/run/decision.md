...
> 状态：**PG0～PG5 已全部通过，Phase 6-1 已关闭（2026-08-06）**。功能与视觉验收、真实 Tauri 手动 E2E 与专项最终 Smoke Test 均由用户确认通过；机器基线 0、批准例外 0。四项跨页耦合无法在本页关闭，已登记为 Phase 6-2 准入前提
...
第 20 节只关闭了组件架构债务，用户体验与视觉确认一直挂着。本轮先取证再评审：用真实源码在 1600/1280/900 三个尺寸、亮暗主题与空闲/运行中/端口占用/历史弹窗各状态抓图，随后出完整页面原型让用户在真实视觉下选型，最终确定「质感表面卡片 + eyebrow 分组标题 + 概览摘要条 + 卡片等高」。本轮不改启动、停止、重启、批量停止、端口释放、配置弹窗、日志与运行历史的任何业务契约。

诊断出的主要问题是层层框中框与大面积语义上色：项目卡内部的命令框和状态框各自带完整边框，状态框还铺 10%~12% 的语义底色，整组又被 raised 外框包住；按钮被 `flex: 1 1 auto` 拉成通栏色块；概览三张大卡用 28px 数字占掉一整行；长路径从左截断把区分项目的尾段吃掉。

- 项目卡接入 `BaseEntityCard`。命令区改左竖线加极淡面，状态改「一个语义点 + 主行 + 细节行」，卡片内部不再有完整四边框的展示型容器。
- 四种状态（运行中 / 启动中 / 端口占用 / 空闲）统一成固定两行，卡片高度因此与状态无关；原来异常态会把内容顶出卡片被 `overflow: hidden` 裁掉底部按钮，该缺陷一并修复。重复的端口占用提示条去掉，占用进程与 PID 落在状态细节行。
- emoji 图标全部换成描边 SVG，跨系统渲染一致；按钮按内容宽右对齐，底部左侧改放模块规模与 Node 版本摘要。
- 路径经新增的 `splitProjectPath` 拆成目录段与尾段，只让目录段省略，项目目录名始终完整。
- 分组标题改 eyebrow 加延伸细线并去掉 raised 外框，与双因验证统一；概览改一行摘要条；组间间距大于组内，网格 `align-items: stretch`。

公共能力扩展先于页面接入完成，`statusPlacement="body"`、`statusTone`、`statusDetail`、`surface="sheen"`、`fillHeight`、`subtitleVariant` 与 `subtitleTail` 均已同步共享清单、组件预览和 smoke test。页面只用公开契约定制，未引用公共组件内部类。

Run Playwright 扩到 15 项，新增覆盖三种状态同排等高与卡片内无框中框；`splitProjectPath` 附 4 项单测。完整回归通过 `npm run lint`、`npm test`（36 个测试文件 / 198 项测试 + 架构门禁 4 项）、`npm run typecheck`、`npm run build:frontend`、Run + Twofa Playwright 29/29，架构基线与批准例外均为 0。

视觉方案已由用户在原型阶段确认。真实 Tauri 中的体验复测与真实进程 E2E 仍是独立 Gate，未关闭前不进入 Phase 6-2 Deploy。

## Run 页面视觉收口（2026-08-06）

本轮只做视觉与信息层级，不改启动、停止、重启、批量停止、端口释放、配置弹窗、日志与分组排序等已确认的业务契约。方案经三轮原型评审确定为「分区容器分组 + 质感增强卡片」。

两次方向修正值得记下：第一轮把带边框的信息 badge 判成框中框、把按钮压成按内容宽，用户对比旧部署面板后指出效果变差；复核确认小尺寸信息标签是排版元素而非嵌套容器，按钮的问题是通栏拉伸而非尺寸本身。第二轮恢复后，用户又指出分组标题与卡片不成整体，遂改为分区容器。

- 分组接入 `BaseDisclosure` 新增的 `panel` 变体：标题条与卡片网格共享一个外框，标题恢复主文本字号，数量做成胶囊徽标，卡片圆角收一档形成嵌套。
- 项目卡改由 `BaseEntityCard` 承载：四种状态统一为「主行 + 细节行」的固定两行状态区，卡片高度与状态无关；表面受光、图标内阴影、命令区竖线带动作色提供质感。
- 工具与版本合并为双段徽标，模块名列前 3 个 + `+N`，消除原先三处重复的模块数量；单体项目用启动命令补位保持行数一致。
- 底部按钮恢复 32px 定高与 82px 弹性基准并加 148px 上限，主操作保有视觉权重且不成为通栏色块。
- 状态不再使用大面积语义底色，只保留语义点与主行文字着色；概览三张大卡收成一行摘要条。
